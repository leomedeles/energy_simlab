...
def run_with_viewers(viewer_count: int, drain_pattern: str) -> tuple[bytes, int]:
    ...
    owner = IntegratedRuntimeOwner(runtime=runtime, publication_sink=fanout)
    ...
    for item in commands:
        owner.run_until(item.apply_tick)
        owner.advance_one_macro((item,))
        if drain_pattern == "fast":
            for viewer in viewers:
                viewer.pop_nowait()
        elif drain_pattern == "mixed" and owner.current_tick % 20 == 0:
...
def test_zero_one_multiple_viewers_and_read_rates_do_not_change_canonical_domain_trace():
    ...
    assert zero[1] == one_fast[1] == one_slow[1] == multiple_mixed[1] == 9
